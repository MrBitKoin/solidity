...
TypePointer TypeChecker::typeCheckABIDecodeAndRetrieveReturnType(FunctionCall const& _functionCall, bool _abiEncoderV2)
{
	...
	// We need to retrieve the expression to obtain source locations.
	...
	while (tupleExpression && tupleExpression->components().size() == 1)
		tupleExpression = dynamic_cast<TupleExpression const*>(tupleExpression->components().front().get());
	...
	for (size_t i = 0; i < tupleType->components().size(); ++i)
	{
		if (TypeType const* argTypeType = dynamic_cast<TypeType const*>(tupleType->components()[i].get()))
		{
			...
			if (actualType->dataStoredIn(DataLocation::Storage))
				m_errorReporter.typeError(
					tupleExpression->components()[i]->location(),
					"Decoding storage pointers is not yet supported."
				);
			else if (!actualType->fullEncodingType(false, _abiEncoderV2, false))
				m_errorReporter.typeError(
					tupleExpression->components()[i]->location(),
